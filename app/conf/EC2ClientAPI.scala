...
class EC2ClientAPI(scaling: AmazonAutoScalingClient, ec2: AmazonEC2Client) {
  ...
  def getSiblingInstancesIps(): Try[List[String]] = {
    def getGroupName(instanceId: String): Try[String] = Try {
      val result = scaling.describeAutoScalingInstances(
        new DescribeAutoScalingInstancesRequest {
          setInstanceIds(Seq(instanceId).asJava)
        }
      )
      result.getAutoScalingInstances.asScala.head.getAutoScalingGroupName
    }

    def getGroupInstanceIds(groupName: String): Try[List[String]] = Try {
      val result =
        scaling.describeAutoScalingGroups(new DescribeAutoScalingGroupsRequest {
          setAutoScalingGroupNames(Seq(groupName).asJava)
        })
      result.getAutoScalingGroups.asScala.head.getInstances.asScala.toList map (_.getInstanceId)
    }
    ...
    val result: Try[List[String]] =
      for {
        ...
        instanceIds map getInstance collect {
          case Success(instance) if isRunning(instance) =>
    ...
    result.recover {
      case e: Throwable =>
        throw EC2ClientAPIException(
          "Error while retrieving sibling instances ip",
          e
        )
    }
  }
  ...
  def getCurrentInstanceIp(): Try[String] = {
    ...
    result.recover {
      case e: Throwable =>
        throw EC2ClientAPIException(
          "Error while retrieving current instance ip",
          e
        )
    }
  }
  ...
  private def getInstanceId(): Try[String] = Try {
    ...
    val conn =
      new URL("http://169.254.169.254/latest/meta-data/instance-id").openConnection
  ...
  private def getInstance(id: String): Try[Instance] = Try {
    val result = ec2.describeInstances(new DescribeInstancesRequest {
      setInstanceIds(Seq(id).asJava)
    })
    result.getReservations.asScala.head.getInstances.asScala.head
  }
...
object EC2ClientAPI {
  ...
  case class EC2ClientAPIException(msg: String, cause: Throwable)
